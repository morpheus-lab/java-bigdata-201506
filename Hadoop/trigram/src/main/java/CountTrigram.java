...
public class CountTrigram {
	...
	public static class MyMapper extends Mapper<Text, Text, Text, LongWritable> {
		...
		public void map(Text key, Text value,
			...
			if (tokenizer.countTokens() >= 3) {
				String firstWord = tokenizer.nextToken().toLowerCase();
				String secondWord = tokenizer.nextToken().toLowerCase();
				while (tokenizer.hasMoreTokens()) {
					String thirdWord = tokenizer.nextToken().toLowerCase();
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 3) {
			System.err.println("Usage: CountTrigram {input path} {ouput path} {topN}");
			return;
		}
		...
		job.setMapperClass(CountTrigram.MyMapper.class);		// 맵퍼 클래스 지정
		job.setCombinerClass(CountTrigram.MyReducer.class);	// 컴바이너 클래스 지정
		job.setReducerClass(CountTrigram.MyReducer.class);	// 리듀서 클래스 지정
		
		// 최종 출력물 키-밸류 형식 지정
		job.setOutputKeyClass(Text.class);				// trigram (연속된 3개의 단어)
		job.setOutputValueClass(LongWritable.class);	// trigram 개수
		...
		// 잡 실행
		if (!job.waitForCompletion(true)) {	// CountTrigram Job
			return;
		}
